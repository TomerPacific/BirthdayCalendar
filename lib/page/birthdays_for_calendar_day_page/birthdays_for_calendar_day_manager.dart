...
class BirthdaysForCalendarDayManager extends ChangeNotifier {
  ...
  UnmodifiableListView<UserBirthday> get birthdays =>
      UnmodifiableListView(_currentBirthdays);
  ...
  void _addBirthdayToList(UserBirthday userBirthday) {
    ...
    List<UserBirthday> birthdaysMatchingDate = _currentBirthdays
        .where((element) => element.birthdayDate == userBirthday.birthdayDate)
        .toList();
  ...
  void removeBirthdayFromList(UserBirthday birthdayToRemove) async {
    ...
    List<UserBirthday> birthdaysForDateDeleted = await storageService
        .getBirthdaysForDate(birthdayToRemove.birthdayDate, false);

    List<UserBirthday> filtered = birthdaysForDateDeleted
        .where((element) => !element.equals(birthdayToRemove))
        .toList();

    storageService.saveBirthdaysForDate(
        birthdayToRemove.birthdayDate, filtered);
    notifyListeners();
  }

  void handleAddBirthdayBtnPressed(BuildContext context, DateTime dateOfDay,
      StorageService storageService) async {
    var result = await showDialog(
        context: context,
        builder: (_) => AddBirthdayForm(
            dateOfDay: dateOfDay, storageService: storageService));
    if (result != null) {
